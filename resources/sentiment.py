...
# def run_model(cleaned_df):
#     filtered = cleaned_df[['cleaned']].query("(cleaned!='')&(cleaned!='blank')")['cleaned'].astype(str)
#     max_features = 50000
#     tokenizer = Tokenizer(num_words=max_features)
#     tokenizer.fit_on_texts(list(filtered))
#     list_tokenized_test = tokenizer.texts_to_sequences(filtered)
#     maxlen = max([len(i) for i in list_tokenized_test])
#     X_te = pad_sequences(list_tokenized_test, maxlen=maxlen)
#     model = tf.keras.models.load_model(r".\resources\lstm_v2.h5")
#     model2 = tf.keras.models.load_model(r".\resources\cnn_1_v2.h5")
#     model3 = tf.keras.models.load_model(r".\resources\cnn_gru_1_v2.h5")
#     model4= tf.keras.models.load_model(r".\resources\bi_gru_1_v2.h5")
    
#     proba = [model.predict([X_te], batch_size=1024, verbose=1),
#         model2.predict([X_te], batch_size=1024, verbose=1),
#         model3.predict([X_te], batch_size=1024, verbose=1),
#         model4.predict([X_te], batch_size=1024, verbose=1)]
#     prediction = pd.DataFrame(np.mean(proba, axis=0))\
#                 .apply(np.argmax, axis=1)\
#                 .map({0:'Negative', 1:'Neutral', 2:'Positive'})
#     df = cleaned_df[['sentence', 'cleaned','index']]\
#     	.merge(pd.DataFrame(prediction,
#     	 index=filtered.index)\
#     	.rename(columns={0:'pred'}),
#     			right_index=True,
#     			left_index=True,
